...
function EthBlockItem(props: EthBlockItemProps) {
  ...
  return (
    <Tr>
      <Td>
        <Link color="blue" to={`/block/${block.number}`} as={ReactLink}>
          {block.number}
        </Link>
      </Td>
      ...
      <Td title={`${utils.commify(block.weiBaseFee)} wei`}>{weiBaseFeeFormatted}</Td>
      <Td title={`${utils.commify(block.weiBurned)} wei`}>{weiBurnedFormatted}</Td>
    </Tr>
  )
}
